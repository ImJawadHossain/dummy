...
(function () {
  ...
  function resetSchedule() {
    try {
      const defStages  = [1*MIN_PER_DAY,3*MIN_PER_DAY,7*MIN_PER_DAY,14*MIN_PER_DAY, 21*MIN_PER_DAY, 30*MIN_PER_DAY];
  ...
  function resetAllProgress() {
    ...
    try {
      ...
      localStorage.removeItem("calculus_formula_knownWords");

      // 2) Clear all scheduled SRS records
      localStorage.removeItem("calculus_formula_srs_v1");

      // 3) Clear any session toast flag (optional)
      localStorage.removeItem("calculus_formula_srs_session_complete");
