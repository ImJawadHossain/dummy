...
function getKnownWords() {
  return JSON.parse(localStorage.getItem("calculus_formula_knownWords") || "[]");
}
...
function saveKnownWord(word) {
  ...
  if (!knownWords.includes(word)) {
    ...
    localStorage.setItem("calculus_formula_knownWords", JSON.stringify(knownWords));
  }
}
...
cardDiv.innerHTML = `
  <div class="card-inner">
    <div class="card-front">
      ...
      <button class="known-icon" onclick="event.stopPropagation(); markAsKnown('${card.reverseOriginal}')">✅</button>
    </div>
...
function showNext() {
  if (currentCardIndex < flashcards.length - 1) {
    ...
    window.location.href = '/calculus_formula'; // redirect to /calculus_formula
  }
}
